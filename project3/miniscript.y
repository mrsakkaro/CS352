...
%token <int_val> NUM
%token SCRIPTYPE SLASHSCRIPT NEWLINE VAR EQUAL COLON SEMICOLON DOCUWRITE OPENPARENT CLOSEPARENT COMMA WS DOT CUROPENBRAC CURCLOSEBRAC PLUS MINUS TIME DIVIDE IF ELSE LESS GREAT LESSEQUAL GREATEQUAL EQEQUAL NOTEQUAL ANDAND OROR EXCLAMATION OPENBRAC CLOSEBRAC ELSEIF
%start parse
%type<expr_val> exp3 exp2 exps exp0 exp1 exp4 exp5
%type<var_val> id
%type<block_val> body
%%

parse 	: SCRIPTYPE {pushBlock();} lines end {
	   vector <Statement * >  block;
	   getTop(block);
...
line 	: VAR ID over{	
				...
				addStatement(new Var_Decl($2, tbl, yylineno));
		}
...
body 	: CUROPENBRAC NEWLINE lines {
	   ...
	   blocks.pop();
	  // $$ = block;
	  }CURCLOSEBRAC NEWLINE
	  	;

id 		:  
	 	ID DOT ID {
			$$ = new ObjectVar($3,new SimpleVar($1, tbl,yylineno), yylineno);

		}
		| ID OPENBRAC exps CLOSEBRAC {
			$$ = new ArrayVar($3, new SimpleVar($1, tbl, yylineno), yylineno);
		} 
		| ID {
			$$ = new SimpleVar($1, tbl, yylineno);
			parent = $<string_val>1;
		}
		| VAR ID {
			SimpleVar * sv = new SimpleVar($2, tbl, yylineno);
			sv -> setDecl(true);
			$$ = sv;
